...
- [ ] Replace web AI solver with EXE-faithful ray marcher: EXE shark.cpp (file 0x24F01-0x2610F) brute-forces trajectories by pixel-level simulation, decrementing gravity passes. Web `solveBallistic()` uses analytic angle/power grid search — different approach that produces structurally different shot selection. Replace with EXE ray march to match difficulty curve and edge-case behavior (wall interactions, wind, terrain occlusion).
...
- [ ] Research Force/Heavy/SuperMag shield behavior from earlier binaries: download "Totally Scorched" pack from whicken.com/scorch/ (contains v1.0, v1.1, v1.2, v1.5). Run struct dump at same file offset (0x05793A) on v1.1/v1.2 EXE — if the linker bug is absent, struct fields (HP, radius, RGB, bhvType) will be intact. Manual gives descriptive hints only (Force=deflects+high HP, Heavy=immune to failures+high HP, Super Mag=best+laser immune+Mag Deflector effects). Use recovered data to fix web port shield config entries for these three items.
...
- [ ] Implement MTN terrain loading (sky type 3): nibble-packed PCX-RLE 4bpp column-major, 72-byte header, 16-color palette from file, height = first non-zero pixel row per column. MTN_PERCENT (default 20.0) = selection probability vs procedural terrain. See "MTN Terrain File Format" section.
