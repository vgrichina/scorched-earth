...
- [ ] Fix noiseParams[i]/100 scaling: ai.js line 217 remove `* noiseParams[i] / 100`. EXE does not scale harmonic output — the amplitude (fraction of budget) already provides appropriate scaling. Test carefully: removing /100 will make AI noise ~100× larger, likely breaking shots. Better: scale by `noiseParams[i] / (NOISE_SHOT_RANGE * 10)` or investigate what DS:3224 provides as scaling constant.
