...
- [ ] Fix Dirt Tower geometry: `addDirtTower(cx, cy, height)` in explosions.js → change `width=3` to `width=45`, loop dy from `−30` to `+20` (EXE: top=−30, bottom=+20 from impact), fill only where terrain pixel is empty (< palette threshold 170). Update `bhvDirt()` `dirtRadius` param if needed.
