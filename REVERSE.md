...
- [ ] Fix AI wind compensation: `_wind` stored (game.js:590) but unused. Implement in `computePowerForAngle`: estimate flight N steps ≈ `|dx|/(cos(angle)×power×MAX_SPEED/1000)`, adjust `target.x` by `−0.5 × WIND_FACTOR × wind × DT × N²`. Skip correction (zero adjust) if wind opposes shot direction. WIND_FACTOR=0.2, DT=0.02 from physics.js.
