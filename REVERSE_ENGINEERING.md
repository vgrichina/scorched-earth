...
- [ ] Add `~` hotkey markers to all submenu item labels: EXE has `~` on every submenu label (e.g., `~Sound:`, `~Flight Sounds:`, `~Interest Rate:`, `~Cash at Start:`, `Computers ~Buy`, `~Air Viscosity:`, `~Gravity:`, `~Borders Extend:`, `~Effect of Walls:`, `~Suspend Dirt:`, `~Sky:`, `~Max. Wind:`, `~Changing Wind`, `~Bumpiness:`, `S~lope:`, `~Flatten Peaks`, `~Random Land`, `Ta~lking Tanks:`, `Talk ~Probability:`, `Tanks ~Fall`, `~Impact Damage`, `~Arms Level:`, `~Bomb Icon:`, `~Tunneling`, `~Scale:`, `Trace ~Paths`, `~Extra Dirt`, `~Useless Items`, `~Mode:`, `Play ~Order:`, `~Teams:`, `~Hostile Environment`, `Status ~Bar`). Web submenu SUBMENUS items have plain labels without `~`. The `drawText()` underline rendering already works — just need to add `~` chars to label strings. Also fix abbreviated labels: "Impact Dmg"→"~Impact Damage", "Hostile Env."→"~Hostile Environment", "Talk Prob.:"→"Talk ~Probability:". **Files**: menu.js
- [ ] Add playfield wind indicator: EXE draws "Wind: N" or "No Wind" + directional pixel-arrow triangle in top-right viewport area (file 0x28F1D, VGA 154). Right-aligned with 20px margin. Arrow: 5-column narrowing triangle at viewportY+10. Separate from HUD — always visible on playfield during turns. Web port only shows wind in HUD rows, no playfield indicator. **Files**: hud.js or game.js (new drawWindIndicator function)
- [ ] Add Borders Extend and Suspend Dirt to Physics submenu: EXE Physics has `~Borders Extend:` (DS:0x5158 EDGES_EXTEND, default 75, pixels) and `~Suspend Dirt:` (DS:0x5166 SUSPEND_DIRT, default 0, 0-100%). Both missing from web Physics submenu. config.js needs `suspendDirt: 0` property; `edgesExtend` not currently in config.js either (needs adding with default 75). **Files**: menu.js, config.js
- [ ] Fix Cash at Start menu max value: Web menu has `max: 100000, step: 5000` but config default is 1000000 (EXE default). The max must be at least 10000000 to allow reasonable configuration. EXE uses `%ld` (long int). Step should be larger for high values (e.g., 50000). **Files**: menu.js (economics submenu startCash item)
- [ ] Fix HUD_MENU_COMPARISON.md 1b Wind Display section: Currently says "format unknown, could include directional arrows". This was RESOLVED in session 10: struct+0xB6 is the player NAME (not wind). The wind display is a separate playfield indicator at file 0x28F1D. Update section 1b to reflect the resolution. **Files**: HUD_MENU_COMPARISON.md
