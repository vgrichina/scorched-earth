...
- [ ] Trace player icon index from sub-struct: EXE uses sub-struct +0x16 to select which of the 48 icon bitmaps (DS:0x3826) to draw per player. Web hud.js uses icon 0 for all players. Disassemble `draw_hud` in icons.cpp around DS:D568+playerIdx×0xCA+0x16 to find what values are stored there and whether they vary by player type/AI level.
...
- [ ] Verify gravity/wind ratio against EXE: web GRAVITY_FACTOR=400, WIND_FACTOR=0.2 gives ratio 2000:1. EXE effective ratio = 2500×G / 1.25×W = 2000:1 at G=0.2 (same). But DS:0x1CF2=50.0 (gravity mult) / DS:0x1CF6=40.0 (wind div) gives step ratio 50/40=1.25 — verify these constants at those DS offsets match documented values, and confirm web scaling k²=0.16 is applied consistently to both terms.
- [ ] Add Simultaneous/Synchronous mode forced x-wrapping in physics.js: EXE (file 0x29CA8) wraps projectiles regardless of wall config when playMode>=1. Fix: in `handleWallCollision()`, check `config.playMode >= 1` and force wrap-around before the wall-type dispatch.

#### AI system (ai.js) — session 107
- [ ] Add AI sticky targeting: EXE `ai_select_target()` at 0x254AF prefers the previous target (sticky), only falls back to random live enemy if previous target is dead. Add `lastTargetIdx` per-player state in ai.js; `selectTarget()` should return previous target if still alive, else pick nearest.
- [ ] Fix Spoiler AI noise budget: EXE Spoiler uses a random budget per solve (unpredictable difficulty). Web ai.js uses fixed `[63, 63, 63]`. Change Spoiler entry to generate random amplitudes each call rather than fixed params.
- [ ] Distinguish Simultaneous vs Synchronous play modes: EXE Simultaneous (1) has a countdown timer (DS:0xD506), AI dispatch flag DS:0x510E=1, and forced projectile x-wrapping. Synchronous (2) is sequential aim + batch fire (no timer, no AI flag). Web collapses both into same SYNC_AIM→SYNC_FIRE path with only `aimTimer` differing. Trace DS:0x510E usage and timer logic to implement correct per-mode AI behavior.
...
- [ ] Audit HUD full mode Row 1 layout: Disassemble compute_hud_layout_full (icons.cpp, DS:0xE9DC-0xE9E0). Verify web hud.js positions for player name (E9DC), weapon X (E9E0 = E9DE+15), power bar, angle bar match EXE formula. Check format strings DS:0x57BA ("Angle"), DS:0x57C5 ("Power"). Update HUD_MENU_COMPARISON.md section.
- [ ] Audit HUD full mode Row 2 — all 7 widgets: Disassemble draw_hud_full Row 2 from 0x304A6. Verify widget positions DS:0xE9EA-0xE9FE, format strings (fuel "%4ld" DS:0x647D, battery "%2d" DS:0x6476, parachute "%2d" DS:0x57D0, shield "%d" DS:0x6476, super mag "%2d" DS:0x57D4, heavy shield "%3d" DS:0x6479), bar widths (20px), icon draw calls. Update HUD_MENU_COMPARISON.md section.
- [ ] Audit HUD basic mode Row 1 — icon draw sequence: Disassemble draw_hud_basic Row 1. Verify draw_icon_alive (0x261D7, flag=1, player color) and draw_icon_dead (0x26245, flag=0, palette 0xA9=169) calls, active player indicator dot, icon index from sub-struct +0x16. Update HUD_MENU_COMPARISON.md section.
- [ ] Audit HUD basic mode Row 2 — bar rendering: Disassemble draw_hud_basic Row 2 from 0x2FC84/0x2FD34. Verify energy bar and angle bar column rendering (bar_column helper 0x39482: 6px×10px columns, fillH clamped 0-10), Y range [HUD_Y+0x0C, HUD_Y+0x17] = 11px tall. Verify web BAR_H=11 and column widths match. Update HUD_MENU_COMPARISON.md section.
- [ ] Audit HUD bars segment (seg 0x3249, base 0x38E90): Disassemble this segment — what functions does it contain? Is all functionality covered by web hud.js? Check whether any bar-drawing primitives here differ from what web implements. Update HUD_MENU_COMPARISON.md section.
- [ ] Audit wind playfield indicator: Disassemble draw_wind_indicator at file 0x28F1D. Verify format string ("Wind: %d" / "No Wind"), X = DS:EF3E (FG_MAXX=screenW-1) - textW - 20, arrow X = FG_MAXX - 15, color palette 154 (40,40,63 medium blue set at 0x2A71A), Y position, arrow shape (5-column triangle). Update HUD_MENU_COMPARISON.md section.
- [ ] Audit HUD drawing color variables DS:0xEF22-0xEF46: Trace how EXE sets DS:0xEF22 (tank_draw_color = player color from palette) and DS:0xEF24 (shadow_color) at runtime during HUD draw. Verify web uses correct palette indices. Note: EF22 is dynamic (set per player), not a fixed palette slot. Update HUD_MENU_COMPARISON.md section.

#### Menu — component audits (session 107)
- [ ] Audit main menu button layout: Disassemble main_menu at 0x3D140. Verify BTN_X (5 small/12 large), BTN_W=80, start_y (5 small/15 large), left panel width formula (2×getBtnX+BTN_W-1), terrain frame bounds (x=getRightX, y=6, maxx=FG_MAXX-6, maxy=FG_MAXY-36). Update HUD_MENU_COMPARISON.md section.
- [ ] Audit main menu title area text colors: Disassemble title rendering at 0x3D6CF. Verify subtitle "The Mother of All Games" color (inherits EF26=white from last emboss layer), "Registered Version" color, copyright color (EF2C=black, set at 0x3D786), "Version 1.50" color. Update HUD_MENU_COMPARISON.md section.
- [ ] Audit player setup screen layout: Disassemble player setup dialog. Verify dialog dimensions, name field width/height, color selector rendering, AI type display, Tab key navigation order, blinking cursor. Update HUD_MENU_COMPARISON.md section.
- [ ] Audit all 10 config submenus — item order, labels, row height: Dump DS string tables for each submenu (Hardware DS:0x28C3, Economics DS:0x291F, Landscape DS:0x296A, Physics DS:0x299A, Play Options DS:0x2A07, Sound, Weapons, etc.). Verify item order, label strings, value name arrays, row height formula (0x3BA92: unconditionally 15 lo-res, 19 hi-res). Update HUD_MENU_COMPARISON.md section.
- [ ] Audit system menu (F9) rendering: Disassemble system menu draw/input at game.js SYSTEM_MENU state. Verify SYSTEM_MENU_OPTIONS list matches EXE DS:0x2B22 pointer table, confirmation texts (DS:0x2C06/0x2BC9/0x2BDE), hotkey letters, row height (14 lo-res / 19 hi-res per seg 0x3F19). Update HUD_MENU_COMPARISON.md section.
- [ ] Audit 3D raised box drawing (draw_3d_box 0x444BB): Verify border widths — bH (left/right vlines) = always 2, bV (top/bottom hlines) = 2 lo-res / 3 hi-res (check at DS:0x6E28==3). Verify corner ownership (hlines claim corners, vlines skip corner rows). Verify fill rect is asymmetric: (minx+2, miny+bV, maxx-2, maxy-bV). Update HUD_MENU_COMPARISON.md section.
- [ ] Audit flat box / sunken box drawing (draw_flat_box 0x44630): Verify 1px borders only (no multi-pixel), corner ownership (TL=Left, TR=Top, BR=Right, BL=Bottom), edge color order (Top=EF30, Left=EF32, Bottom=EF26, Right=EF2E). Update HUD_MENU_COMPARISON.md section.
...
- [ ] Audit shop Miscellaneous sub-categories: Verify 5 headers (Parachutes DS:0x2E5B, Triggers DS:0x2E67, Guidance DS:0x2E71, Shields DS:0x2EFE, Inventory DS:0x2EEF) and exact weapon index ranges for each group. Update HUD_MENU_COMPARISON.md section.
- [ ] Audit shop Score tab: Verify "Player Rankings" title (DS:0x6042) centered, "#%d" rank format (DS:0x6052), score format "%d" (DS:0x6056), row height (11 if screenH<220, 13 if >=220 at 0x341F5). Update HUD_MENU_COMPARISON.md section.
- [ ] Audit shop tab buttons and Done button: Verify tabs render as 3D raised (inactive) / sunken (active) boxes, Done button as 3D raised box, exact dimensions and gap between tabs. Update HUD_MENU_COMPARISON.md section.
- [ ] Audit shop sell dialog: Verify "Sell Equipment" title (DS:0x??), fields ("Quantity to sell:", "Accept", "Reject"), refund formula floor(qty × price × factor / bundle) where factor=0.8 (DS:0x613C) or 0.65 (DS:0x6144) for free market. Update HUD_MENU_COMPARISON.md section.
- [ ] Audit shop scrollbar widget: Verify track/thumb/arrow sizing, 3D box bevel order for up-arrow, down-arrow, and thumb (all raised), drag behavior. Update HUD_MENU_COMPARISON.md section.
- [ ] Audit shop palette animation: Disassemble tick function at file 0x14E34. Verify 3 effects: VGA 2 triangle pulse (100-frame counter, R=tri×63/50, G=tri×10/50), VGA 8-11 accent 4-step rotation (entries 1-4, every 8 frames), VGA 14-18 gray gradient cycling (5 levels 0/15/30/45/60). Update HUD_MENU_COMPARISON.md section.
- [ ] Audit shop AI auto-purchase: Verify 12-case jump table at 0x1DF4D — case 0-3 buy random weapon, case 5 buy shield, case 6 buy defense, case 8 buy guidance, case 9 mountain gear, case 10 sell random, cases 4/7/11 no-op. MTN re-roll condition DS:0x50D8. Update HUD_MENU_COMPARISON.md section.
- [ ] Audit shop cash display: Disassemble cash_display at 0x16A7C. Verify "Cash Left:" label (DS:0x2DDD), "Earned interest" display (DS:0x2EDF), number format, position in shop UI. Update HUD_MENU_COMPARISON.md section.
- [ ] Audit shop "NO KIBITZING!!" privacy guard: Verify trigger condition (hotseat human players), display format (player name shown), any-key dismiss. Update HUD_MENU_COMPARISON.md section.

#### Font, Talk, Round/Game-over — component audits (session 107)
- [ ] Audit font system — glyph rendering and layout modes: Verify all 161 glyphs render correctly (DS:0x70E4-0x94EA, row-major byte-per-pixel). Verify DS:0xED58 layout_mode (0=25px rows, 1=17px rows) switches correctly in HUD. Verify `~` hotkey skip behavior (EXE silently skips; web draws underline — confirm intentional). Update HUD_MENU_COMPARISON.md section.
- [ ] Audit talk bubble rendering: Disassemble display_talk_bubble at 0x182FD. Verify border color EF2C (UI_DEEP_SHADOW=black), fill color EF26 (UI_DARK_BORDER=white), text color EF2C, Y = tank.y - 19, box height 14px, X clamp = FG_MAXX - textW - 11, left padding 3px. Update HUD_MENU_COMPARISON.md section.
- [ ] Audit round-over screen: Disassemble round_over_display at 0x2A95A. Verify winner display (bare player name, no suffix), "No Winner" text, format strings ("%d of %d rounds fought." DS:0x3002, "1 round remains" DS:0x2C2B, "%s rounds remain" DS:0x2C1A), any-key via fg_getkey. Update HUD_MENU_COMPARISON.md section.
- [ ] Audit game-over screen: Verify "Final Scoring" title (DS:0x2A9F), scoring display (bare numbers, no "pts"/"W" — web-only), "<<Press any key>>" (DS:0x5212), any-key via fg_getkey. Update HUD_MENU_COMPARISON.md section.
