...
### Bomb Icon Sizes — VERIFIED (session 136)

Config strings: "Small" DS:0x27B8, "Invisible" DS:0x27C2 (at file 0x58538, 0x58542)
Config key "BOMB_ICON" at DS:0x055B; parsed at file 0x1A5E9.

**DS:0x51A2 = BOMB_ICON**: 0=Small, 1=Big (default), 2=Invisible (transitional only, see below)
**DS:0x514E = BOMB_INVISIBLE_FLAG**: 0=visible, 1=invisible (suppress all projectile head drawing)

**Rendering mechanism** (icons.cpp, file 0x2B471):
- Init function at 0x2B471 sets far-pointer pair DS:0x1C8A/0x1C8C based on BOMB_ICON:
  - BOMB_ICON==0 (Small): ptr = 1A4A:1A3B (file 0x228DB) — draws 1 white pixel (color 0xFF)
  - BOMB_ICON!=0 (Big/Invisible): ptr = 1A4A:1B74 (file 0x22A14) — draws 5-pixel cross
- Then calls equip_init

**Both draw functions** (Small 0x228DB, Big 0x22A14):
1. If param[bp+0x0A]!=0 → erase mode (restore saved terrain pixel under projectile)
2. If DS:0x514E != 0 → skip all drawing (Invisible mode)
3. Check bounds vs SHIELD_DRAW_X_MIN/MAX, SHIELD_DRAW_Y_MIN (DS:EF42/EF3C/EF40)
4. Plot pixels

**Invisible handling**: BOMB_ICON=2 is only a transitional value. The menu cycling code
(0x3D908) converts it: BOMB_ICON→0 (Small), DS:0x514E→1 (invisible flag set). So at
runtime, Invisible = BOMB_ICON=0 + BOMB_INVISIBLE_FLAG=1. The draw pointer (1A4A:1A3B
Small function) is still called but immediately returns without drawing due to DS:0x514E=1.
...
| +0xA6 | 4 | uint32 | max_health | 32-bit max health (bar denominator). Low: `[es:bx+0xA6]`, High: `[es:bx+0xA8]`. Init from DS:0x50E4/E6 at 0x30DD0. |
| +0xAA | 2 | int16 | fuel_energy | Fractional Heavy Shield fuel energy (0–9 within one Fuel Tank unit). Starts at 0; decremented when Heavy Shield absorbs damage; refills by 10 (and decrements inventory[FuelTank]) when it goes negative. Used in `check_heavy_shield_energy` (0x31249): total = `inventory[D564=FuelTank]×10 + struct[+0xAA]`. |
| +0xAC | 2 | int16 | unk_AC | Initialized to 0 in player_init_substruct (0x30F29). Purpose unknown. |
...
MAX_WIND=0
CHANGING_WIND=Off
HOSTILE_ENVIRONMENT=On    # Per-pixel damage when projectile traverses tank body
...
- [x] Identify sub-struct +0xAA field and verify Heavy Shield energy formula: **RESOLVED (session 137)**. `check_heavy_shield_energy` (0x31249) uses DS:0xD564 = **WPN_FUEL_TANK_IDX** (Fuel Tank, index ≈55), NOT Heavy Shield (DS:D55E). Formula: `total = inventory[FuelTank]×10 + struct[+0xAA]`. struct[+0xAA] = **fractional fuel energy** (0-9 per Fuel Tank unit, starts at 0); decrements on damage; refills by +10 and decrements FuelTank count when it goes negative. Sub-struct layout updated: +0xA6/A8 = max_health (32-bit, init from DS:0x50E4/E6 at 0x30DD0), +0xAA = fuel_energy (int16, 0-9 fractional), +0xAC = unk_AC (int16, init 0). **Web port bug fixed**: hud.js Widget 7 was using `inventory[WPN.HEAVY_SHIELD]` and `player.shieldEnergy` (+0x96 absorption HP) — both wrong. Fixed to use `inventory[WPN.FUEL_TANK]` and `player.fuelEnergy` (≡ struct[+0xAA], defaults to 0 since web port doesn't yet implement fractional depletion). **weapons.js**: corrected HEAVY_SHIELD comment (was DS:D564, now DS:D55E), added `FUEL_TANK: 55` constant. **comments.csv**: updated check_heavy_shield_energy and DS:D564 comments to clarify Fuel Tank powers Heavy Shield.
- [x] Trace Force Shield deflection behavior in v1.50: **RESOLVED (session ~140)**. EXE deflection code found in extras.cpp at file 0x23469. Algorithm: compute angle1=atan2(hit_dy,hit_dx) (normal from tank center to hit), angle2=atan2(vy,vx) (incoming velocity), delta=(angle2-angle1)×2, then rotate(-v, delta) — equivalent to standard specular reflection v'=v-2(v·n̂)n̂. DS:0x1C7A set to 1 during deflection. Condition: player.inventory[WPN.FORCE_SHIELD]>0 (weapon 50, DS:D562). Web port: added `WPN.FORCE_SHIELD=50` to weapons.js, implemented `checkShieldDeflection()` in shields.js using specular reflection formula. Force Shield does NOT use absorption energy — deflects as long as inventory>0.
...
- [x] Full fidelity re-audit: systematically read every web/js/*.js file and compare against documented EXE behavior in REVERSE.md. Find discrepancies, fix them, add findings to REVERSE.md and HUD_MENU_COMPARISON.md. Sessions 107–137 fixed many targeted issues (AI noise, physics, shields, terrain, HUD basic mode labels, HOSTILE_ENVIRONMENT, shop stride, system menu row height, etc.). **DONE (session 139)**. Two bugs found and fixed: (1) MIRV sub-warhead radius: `bhvMirv()` returned hardcoded 20 for all sub-warhead explosions, ignoring `proj.subRadius` (35 for Death's Head, 20 for MIRV); fixed to `const radius = (proj.isSubWarhead && proj.subRadius) ? proj.subRadius : 20`. (2) Funky Bomb sub-warhead radius: sub-bombs spawned with custom `subRadius` (random(10)+15)×scale but routed through `bhvStandard` (weaponIdx=2 Baby Missile, param=10) which ignored `subRadius`; fixed by adding sub-warhead override in `handleBehavior` dispatch: if `proj.isSubWarhead && proj.subRadius && result.explode && weapon.bhv === BHV.STANDARD`, override radius with `proj.subRadius`. Also fixed stale comment in config.js: `hostileEnvironment` comment now correctly describes EXE per-pixel traversal damage (DS:0x513C) instead of old fabricated lightning/meteor behavior. Other Gemini-flagged items verified: napalm damage (single pass at end, not stacking), heat guidance constants (GUIDANCE_K=10000, DT=0.02 per DS:0x3224/CEAC verified), linear tank fall (FALL_SPEED=2px/step EXE-faithful). **Files**: web/js/behaviors.js, web/js/config.js.
- [x] Fix HUD basic mode Row 2 labels and second bar: **DONE (session 137)**. Session 111 found three discrepancies never fixed: (1) Label 1: web showed `player.name + ':'` → EXE shows "Max:" (DS:0x2364→DS:0x2EFA, static, no runtime writes); (2) Label 2: web showed "Angle:" → EXE shows "Shields:" (DS:0x2368→DS:0x2EFE); (3) Second bar: web showed angle/18 → EXE uses `sub[0x96]*100/maxEnergy/10` = shield_energy/max_energy*10. Fixed hud.js: added `import { SHIELD_CONFIG } from './shields.js'`, changed L1 to `'Max:'`, L2 to `'Shields:'`, second bar to `shieldEnergy * 10 / maxEnergy` (guards against zero max). Updated Row 2 comment in header. Also fixed SCREEN_HIDE in main.js: added "NO KIBITZING!!" centered text per session 132 finding (EXE file 0x43080, DS:0x2E2A, draw_embossed_text); web additions (player name + "<<Press any key>>") retained for usability. **Files**: web/js/hud.js, web/js/main.js
...
### RE Investigation (from session 141)
- [x] AI behavior audit (shark.cpp vs web/js/ai.js): **DONE**. Full cross-reference. Key findings:
  - **Core solver**: EXE pixel-level ray-marching via `fg_getpixel`; web uses closed-form ballistic formula — permanent architectural difference, not fixable in browser. Web proxy is practically sound.
  - **Wind compensation MISSING** (Medium): `setAIWind()` called (game.js:590,950) but `_wind` never used in `computePowerForAngle`. EXE `ai_wind_correction` (file 0x2589C): checks if wind opposes direction → aborts; else applies `correction = DS:3224 × gravity / dist^0.25` to vel_x/vel_y.
  - **noiseParams[i]/100 scaling** (Medium): ai.js line 217 `* noiseParams[i] / 100` has no EXE basis. EXE passes amplitude directly; controls harmonic budget and `freq_base = π/(2×amp)` only; no post-hoc rescale.
  - **shot_number domain**: EXE increments per target-column within one call (outer scan loop); web increments per turn. Tied to ray-march architecture — not independently fixable.
  - **Confirmed correct**: Spoiler noise `[random(2), random(100), random(100)]`, phase range `random(300)`, harmonic formula `amp*sin(freq*(phase+shotNum))`, Cyborg/Unknown random(6) dispatch (type 0 edge handled by fallback), target selection (Euclidean distance correct; DS:0x5186=40 is HEAT GUIDANCE threshold, already in behaviors.js HEAT_PROXIMITY=40, not general AI targeting).
- [x] Fix AI wind compensation: **DONE** (session 142). Implemented in `aiComputeShot` harmonic branch (ai.js). When wind aids shot direction (sign(dx) === sign(_wind)): estimate rawPower first with unmodified target, compute hVel = cos(angle)*rawPower*MAX_SPEED/1000, flightTime = |dx|/hVel, windDisp = 0.5*WIND_FACTOR*wind*flightTime², adjust windTarget.x = target.x − windDisp, then recompute power with corrected target. Skip entirely if wind opposes or is zero (matches EXE conditional behavior). WIND_FACTOR=0.2, MAX_SPEED=400 match physics.js.
- [x] Fix noiseParams[i]/100 scaling: **session 143**. **CORRECTED UNDERSTANDING**: `play_ai_noise_params` at file 0x29505 (REVERSE.md labeled it "AI noise params" but it's WRONG) is actually a **VGA palette setter** — it calls `fg_setrgb(0x9D, R, G, B)` to set palette entry 157 to an AI-type-dependent color. Values [50,23,63] are RGB color components, NOT noise amplitudes. DS:3224=10000.0 is a wind-correction constant, unrelated to noise. EXE harmonic loop at 0x26006-0x26054 confirmed: `angle = base_angle + SUM(amp[i]*sin(freq[i]*(phase[i]+shotNum)))` — no extra scaling. **Fix applied**: ai.js line 217 changed from `* noiseParams[i] / 100` to `/ noiseState.generators.length` — dividing by generator count normalizes total amplitude to ≈ EXE's single-generator range (±30°). Frequency still differs by type via freq_base = π/(2*noiseParam), preserving accuracy ordering.
- [x] Weapon behavior audit (Napalm, Dirt, Laser): **DONE**. Key findings:
  - **Dirt Tower width HIGH**: EXE file 0x2DF00: ±45px horizontal (91px wide), −30 to +20 vertically from impact, fills only where pixel < 170 (empty). Web `addDirtTower()` uses width=3 (7px), upward only 30px. Fix: width=45, dy=−30..+20.
  - **Napalm fire impact explosion Medium**: EXE: visual-only `fg_drect(60,20,20,fire_color)` flash on impact, NO real explosion. Web `bhvNapalm()` line 326: `explode: !isDirt, radius: 5` — applies real radius-5 damage at impact in fire mode. Fix: `explode: false, radius: 0` for fire mode.
  - **Dirt napalm color Medium**: EXE DS:E702=0x50=80 for dirt particle pixels. Web `DIRT_NAPALM_COLOR=130`. Wrong color affects self-exclusion check in particle walk. Fix: change to 80; update game.js cleanup check `cur === 130` → `cur === 80`.
  - **Confirmed correct**: Napalm particle walk logic, damage ranges (Hot: maxDmg=50 range=40; Regular: maxDmg=30 range=25), wind lateral spread `wind > 0 ? 1 : -1`, explosion recording every 20 steps, 1000-step max. Laser: pure accessory, no functional discrepancies (color remap intentional).
- [x] Fix Dirt Tower geometry: **DONE (session 144)**. `addDirtTower()` in explosions.js: `width` changed 3→45 (EXE ±45px horizontal, file 0x2DF00), loop changed from `dy=0..height` upward to `dy=−30..+20` relative to impact (EXE top=−30, bottom=+20). Fill condition kept `!terrainBitmap` (EXE: pixel < 170). `bhvDirt()` `dirtRadius` param unchanged (still passed but now unused by fixed geometry).
- [x] Fix Napalm fire impact explosion: **DONE (session 144)**. `bhvNapalm()` behaviors.js: changed `explode: !isDirt, radius: isDirt ? 0 : 5` → `explode: false, radius: 0`. EXE does visual-only `fg_drect` flash, no actual explosion damage at impact. **Files**: behaviors.js
- [x] Fix Dirt napalm particle color: **DONE (session 144)**. behaviors.js `DIRT_NAPALM_COLOR = 130` → `80` (EXE DS:E702=0x50=80). game.js cleanup: `cur === 130` → `cur === 80`, `setPixel(...130)` → `setPixel(...80)`. **Files**: behaviors.js, game.js
- [x] Round-over/game-over screen audit: **DONE**. Session 135 already covered the main structure. New details confirmed:
  ...
  - **Row height**: 11px if `FG_MAXY < 220`, else 13px (confirmed at file 0x341F5). Session 135 had this correct.
  - **Key functions**: `end_of_round_scoring` 0x33FC3, `find_winner_and_display` 0x340AF, `Final Scoring setup` 0x34F2F, `Final Scoring paint` 0x34CC6, `"rounds fought" paint` 0x42D19, `score_sort_players` 0x3364F.
...
- [x] Fix stale config.js comment: **DONE (session 139)**. `hostileEnvironment: 1` comment previously read "(random lightning/meteor events)" — the old fabricated web behavior replaced in session 138. Updated to "(DS:0x513C) — 1 damage per tank pixel traversed by projectile". **Files**: web/js/config.js.

### RE Investigation (from session 138)
- [x] Fix HOSTILE_ENVIRONMENT implementation: **DONE (session 138)**. EXE behavior (file 0x3EB72, menu module seg 0x34ED): per-pixel projectile traversal checks `pixel < 0x50 (80)` = tank pixel. If HOSTILE_ENVIRONMENT (DS:0x513C) is On → `shield_and_damage(tank_ptr, 10, 1)` deals 1 damage per tank pixel traversed; projectile CONTINUES (return AX=0). Web port had completely fabricated behavior: `random(100)<10` per-turn "lightning or meteor" events (lightning damages nearby tanks, meteor spawns a Baby Missile from above). **Web fix**: (1) physics.js `stepSingleProjectile` — when `config.hostileEnvironment && pixel ∈ [1,80)`: find player from `Math.floor(pixel/8)`, apply 1 damage to `players[pIdx].energy`, return `'flying'` (continue); (2) game.js — removed the per-turn lightning/meteor block; (3) main.js — removed lightning bolt rendering; (4) game state — removed `hostileLightningX`/`hostileLightningFrames` fields; (5) sound.js `playLightningSound` stub kept (still exported, no callers). REVERSE.md SCORCH.CFG comment corrected. **Files**: web/js/physics.js, web/js/game.js, web/js/main.js, REVERSE.md.

### RE Investigation (from session 137)
- [x] Trace BOMB_ICON effect on projectile rendering: **DONE** (session 136). DS:0x51A2=BOMB_ICON (0=Small, 1=Big, 2=Invisible-transitional), DS:0x514E=BOMB_INVISIBLE_FLAG. Init at 0x2B471 sets draw far-ptr: Small→1A4A:1A3B (1px dot, file 0x228DB), Big→1A4A:1B74 (5px cross, file 0x22A14). Both functions check DS:0x514E==1 and return without drawing (Invisible mode). Invisible at runtime = BOMB_ICON=0 + DS:0x514E=1 (set by menu cycling at 0x3D908). Web fix: `drawAllProjectiles` in main.js now checks `config.bombIcon` — skip head drawing if 2 (Invisible), draw single pixel if 0 (Small), draw cross if 1 (Big). Labels added: DS:0x514E=BOMB_INVISIBLE_FLAG.
- [x] Trace TUNNELLING effect on physics: **DONE** (session 136). DS:0x513A=TUNNELLING (0=Off, 1=On). Checked in extras.cpp terrain-collision handler at file 0x224F7. When TUNNELLING=1 AND projectile is not homing (DS:E1E4/E1E6 == 0:0) AND shooter's player[+0x4A]!=0 AND no super_mag override (DS:1C76==0): applies MAG_DAMP_COEFF (DS:1D54=0.75) × velocity each terrain hit; absorbs (no explosion) if vx²+vy² < MAG_ABSORB_THRESHOLD (DS:1D58=2000.0). Tunneling borrows the same damping constants as the Mag Deflector shield. Homing missiles (Heat Guidance) cannot tunnel. Web fix: physics.js `stepSingleProjectile` terrain hit branch calls `applyMagDamping` instead of returning `hit_terrain` when `config.tunneling` is true and `!proj.heatTarget`.
- [x] Trace USELESS_ITEMS effect on shop: **DONE (session 137)**. DS:0x50FE = USELESS_ITEMS is a **dead config flag in v1.50** — it has no gameplay effect on the shop. Full audit: (1) Only read via `FF 36 FE 50` (PUSH [0x50FE]) at 0x3C677 when building the PlayOptions config dialog to display current checkbox value; (2) Written back via `A3 FE 50` at 0x3CF3A after user toggles the "~Useless Items" checkbox in PlayOptions dialog; (3) Also read in config writer at 0x1B02E and 0x1D818 to control CFG file output format. (4) Exhaustive search for actual gameplay reads: `A1 FE 50` (MOV AX) = 0 hits; `8A 06 FE 50` (MOV AL) = 0 hits; `83 3E FE 50` (CMP [50FE],imm) = only 2 hits, both in config writer; `8B 06/1E FE 50` = 0 hits; `FF 36 FE 50` = only 1 hit (dialog builder). All "(CMP)" annotations from xref.py were false positives (`cmp si, 0x0050` = bytes 83 FE 50). (5) The dialog toggle handler at 0x3F19:0x17D0 (file 0x47360) is a draw callback, not a gameplay filter. **Conclusion**: USELESS_ITEMS is a planned-but-never-implemented or removed feature in v1.50. The web port correctly needs no USELESS_ITEMS shop filter. No web fix needed.
- [x] Trace FLATLAND effect on terrain generation: **DONE (session 138)**. DS:0x5172=FLATLAND (config key "FLATLAND", DS:0x048F). The FLATLAND flag maps to `LandGenerator.flatten_peaks` (+0x0E). In the random walk kernel (file 0x29808), after computing `walk_delta`, if `flatten_peaks && (walk_delta < -3 || walk_delta > 3)` → `walk_delta = 0`. This suppresses extreme slope steps, creating flatter terrain. FLATLAND is NOT directly read in terrain_gen_main — it's set into the LandGenerator struct before calling the walk kernel. The terrain random-init at file 0x29562 (1F7F:3372) calls `rand(2)→DS:5172, rand(100)→DS:516E, rand(100)→DS:5170` each round (RANDOM_LAND behavior). Also corrected: bumpChance in generateRolling was using `config.land1` instead of `config.land2`. **Web fix**: terrain.js `generateRolling()` — changed `bumpChance = config.land2` (was `config.land1`), added `if (flattenPeaks && (walkDelta < -3 || walkDelta > 3)) walkDelta = 0`. Also corrected REVERSE.md LAND1/LAND2/FLATLAND DS offset table (was wrong). **Files**: web/js/terrain.js, REVERSE.md.
- [x] Trace USELESS_ITEMS shop filter: **DONE (session 137)** — see above. Dead flag, no shop filter in EXE.
- [x] Fix labels.csv: DS:0x5172 already labeled "FLATLAND" — confirmed correct (config key "FLATLAND" at DS:0x048F, config parser writes DS:0x5172 at file 0x1A14B). No change needed.
...
| Variable | DS Offset | Config Key | Range | Default | Maps to |
|----------|-----------|-----------|-------|---------|---------|
| LAND1 (bumpiness) | DS:516E | "LAND1" (DS:0483) | 0-100 | 20 | flat_chance (+0x0A): % maintain momentum |
| LAND2 (slope) | DS:5170 | "LAND2" (DS:0489) | 0-100 | 20 | bump_chance (+0x0C): % double delta |
| FLATLAND | DS:5172 | "FLATLAND" (DS:048F) | 0-1 | 0 | flatten_peaks (+0x0E): suppress \|delta\|>3 to 0 |

Config parser writes at file 0x1A057 (LAND1), 0x1A0CD (LAND2), 0x1A14B (FLATLAND). Config writer reads at file 0x1AD81.

**RANDOM_LAND** (file 0x29562, 1F7F:3372): Each round randomizes FLATLAND=rand(2), LAND1=rand(100), LAND2=rand(100).
...
- [x] **Pixel audit**: menu layout constants — **VERIFIED** (session 156). EXE main_menu at 0x3D140: btnX=5(small)/12(large), startY=5/15, BTN_W=80, rowH=17/25 (DS:0x6316 table). terrain_frame_x = 2*btnX+79 = 89/103. Frame: (89,6)→(313,163) for 320×200. Dialog width=100. All match web: getBtnX(), getStartY(), BTN_W=80, getRowH(), getRightX(). Outer 3D box = full screen. Frame right = FG_MAXX-6, bottom = FG_MAXY-36. Copyright width check triggers shorter frame if text too wide (never in standard modes). No fixes needed.
- [x] **Pixel audit**: menu title "Scorched Earth" + "The Mother of All Games" — **VERIFIED** (session 156). EXE: embossed title at 0x3D6CF, Y=2(small)/11(large), centering uses 2*textW+4. Subtitle Y=27/41 in EF26 color. "Registered Version" Y=52/71. Web matches all Y coords, emboss layers, and centering formula.
- [x] **Pixel audit**: "Version 1.50" + copyright footer — **VERIFIED** (session 156). EXE at 0x3D790: copyrightY = FG_MAXY-20 = screenH-21, versionY = copyrightY-13, color = EF2C (deep shadow). Wide copyright splits to 2 lines (sub 0x0D). Web matches: copyrightY=screenH-21, versionY=copyrightY-13, UI_DEEP_SHADOW color, 2-line split logic.
...
- [x] **Pixel audit**: HUD row heights — **VERIFIED** (session 153). EXE: HUD_Y=5 (DS:518E), ROW2_Y=HUD_Y+0xC(12). Web: HUD_Y=5, ROW2_Y=HUD_Y+12. Layout mode (DS:ED58) affects menu/config rows only (compact=5px, spacious=12/15px), not in-game HUD. Bar column 6px×10px fill, 12px total with outline. All match.
...
- [x] **Pixel audit**: power/angle bar fill columns — **VERIFIED** (session 153). EXE bar_column (0x39482): 6px wide (X..X+5), 10px fill area, clamped 0-10, bottom-up fill, EF2C background. Outline: draw_flat_box at (barX-1, y, barX+numP*6, y+11) = 12px total. Web drawBarColumn: 6px wide, BAR_H=11+outline=12px, clamped to 10px interior. Exact match, only fixed stale comment (0x394F2→0x39482).
...
- [x] Current web skips player setup dialogs (auto-names) — **FIXED** (sessions 146-147). Player setup dialog fully implemented: name field, color row, Human/Computer toggle, Done button, sequential per-player flow. See section 2 above.
...
- [x] **Font rendering**: **VERIFIED** (session 154). font_verify.py reads EXE font pointer table (font_init at 0x4C290, 161 custom glyphs, formula DS:[char*4-0xCA6]→far ptr→width+bitmap). All 94 ASCII glyphs (32-126 minus 34) have identical widths AND pixel-perfect bitmaps vs web font.js. All 126 extended glyphs (0x80-0xFD) also match exactly. Only discrepancy: char 34 (`"`) has width=0 in EXE (Fastgraph font omission) vs width=3 in web (handcrafted improvement). measureText spacing (width+1 per char, skip `~`) matches EXE text_measure at 0x4CE17. No fixes needed.
...
- [x] **Palette accuracy**: **VERIFIED** (session 153). Extracted all 210 fg_setrgb call sites from EXE. All gameplay-critical palette entries match web: UI colors (151-159→web 200-208), explosion fire (170-199, formula si*2+43/si+10 exact match), wall (150=50,50,50), wind color (154=40,40,63), player gradients (0-79), laser (253-254). Unset in web but harmless: VGA 157 (menu-only grey 50,50,50), VGA 160-161 (font init colors, overwritten), VGA 230-232 (shield flash teal/green/blue, cosmetic), VGA 250 (menu sky preview orange), VGA 255 (white, unused by web). No fixes needed — web palette is accurate for all visible rendering.
...
- [x] **Dialog framework**: **ANALYZED** (session 155). EXE dialog system at segment 0x3F19 (file base 0x45B90). Key functions: dialog_create (0x45C72, allocs 0x14C=332 bytes), dialog_position (0x45D2C, centers on screen), dialog_draw (0x45DDC), dialog_run (0x45FED). Dialog struct: +0x00=x, +0x02=y, +0x04=width, +0x06=height, +0x08=item_count, +0x0E=flags, items at +0x20 (far ptrs, stride 4), hidden flags at +0xE8 (stride 2, up to 100 items). Widget types: 0=text, 3=list, 8=button. Centering: `(screen - margin - dialog_size) / 2` — matches web's `Math.floor()`. **Submenu dialog**: hard-coded 310×140 at (110,50) via `dialog_create(0x6E, 0x32, 0x0136, 0x008C)`. Row height=15 (unconditional). Items placed at (x=10, y=5+i*15). Web: dynamically computes width from content, height=30+N*rowH, items at y+18+i*rowH — structurally different but functionally equivalent (web adds title+separator that EXE handles inside list widget). **Confirm dialog**: text width padded +20px (0x14), button gap +30px (0x1E), height=53 (0x35), text at y=10, buttons at y=27. **draw_3d_box** (0x444BB): 2px left/right borders, 2px (lo-res) or 3px (hi-res, FG_PAGES==3) top/bottom borders; colors LEFT=EF26, TOP=EF2E, RIGHT=EF30, BOTTOM=EF32; fill inset by (2,bV). Web drawBox3DRaised matches geometry and color assignments. **draw_border** (0x1826C): 1px border in EF2C + interior fill in EF26; used for simple flat frames. No web port fixes needed — dialog layout is functionally correct.
- [x] **Explosion rendering**: **ANALYZED** (session 153). EXE uses Bresenham filled circle algorithm (0x43690) expanding over N frames. draw_explosion_expanding (0x2476B) receives (cx, cy, palette_base=170, num_frames=20). Each frame: palette_base+=6→176, ring_inner=176-(frame*6/N), circle drawn at expanding radius with fire palette. Per-pixel check: skips pixels already in fire range [inner,outer] to prevent overdraw. Color 0x50 (80) triggers setTerrainPixel for terrain-aware drawing. Web uses trig-based ring (cos/sin at 3° steps) with ringWidth=30% of radius, inner=attacker color, outer=fire palette — visually similar, algorithmically different. Acceptable divergence for reimplementation.
- [x] **Projectile trail**: **VERIFIED** (session 153). EXE draws projectile head as VGA 255 (white), only over pixels >0x77 (terrain range). Small bomb = 1px, Big bomb = 5px cross (center + 4 cardinal). NO color-fading trail in EXE — when Trace Paths=off, previous positions are immediately restored; when on, white pixels remain permanently. Web adds cosmetic age-fading (white→orange→dark) which is an enhancement not in EXE — acceptable divergence. Erase logic restores terrain via setTerrainPixel (for laser color 0x78) or draws stored pixel color. Big cross pattern matches web BOMB_ICON handling.
