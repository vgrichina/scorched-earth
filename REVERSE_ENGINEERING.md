...
- [ ] Investigate round-over winner display format: Web main.js:249 shows `name + ' wins!'` but EXE has no "wins!" or "%s wins" string — only "No Winner" (DS:0x2CE6). The actual winner text rendering is inside a dialog paint callback reached via MZ-relocated far call at file 0x2A98C (`call far 0x0000:0x164F` with params DS:0x5104/0x5106). Need to resolve the relocation target or capture the display from v86 emulator to determine actual format. **Files**: main.js
