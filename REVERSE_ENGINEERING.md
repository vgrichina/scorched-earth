...
- [ ] Fix submenu dialog width — hardcoded 220px: EXE dialog system (seg 0x3F19) computes width from content. Web uses `dlgW = 220` for all submenus. Fix: compute from `max(measureText(title), max(measureText(label) + measureText(value)))` + padding. **Files**: menu.js (drawSubmenu)
- [ ] Add shop Miscellaneous sub-category headers: EXE groups misc items under 5 headers — "Parachutes" (DS:0x2E5B), "Triggers" (DS:0x2E67), "Guidance" (DS:0x2E71), "Shields" (DS:0x2EFE), "Inventory" (DS:0x2EEF). Web shows flat list. Fix: insert non-selectable header rows with hline separators between category groups. **Files**: shop.js (updateItemList, drawShop)
- [ ] Fix shop tab buttons — plain text vs 3D boxes: EXE renders tabs as 3D raised boxes (active=sunken) via dialog widget system. Web draws tab names as plain text with underline for active. Fix: use drawBox3DRaised/Sunken for each tab, text centered inside. **Files**: shop.js (drawShop tab bar)
- [ ] Fix embossed title X-position — manual fudge vs computed: EXE centers "Scorched Earth" in right panel accounting for 4px emboss shift. Web uses `centerXRight(titleStr) - 2` hardcoded. Fix: derive offset from emboss layer count (5 layers → 4px shift → offset = 2). **Files**: menu.js (drawMainMenu)
...
- [ ] Remove `< >` arrow display on spinner values: Web shows `< N >` around spinner values when selected — not in EXE. EXE uses keyboard left/right arrows without visible `< >` indicators. Fix: show just the value right-aligned. **Files**: menu.js (drawMainMenu spinner)
